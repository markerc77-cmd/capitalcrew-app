export default async function handler(req, res) {
  ...
  // ── 8 PARALLELE FETCHES (optimiert für Vercel 10s Timeout) ──
  const [yahooRaw, profileRaw, ratiosRaw, incomeRaw,
         earningsRaw, newsRaw, priceHistRaw, quoteRaw] = await Promise.all([
  ...
  const exchange = meta.fullExchangeName || 'NASDAQ';
  ...
  // Kurs Fallbacks
  const liveKursFinal = liveKurs || fmpQuote?.price || prof?.price || 0;
  const w52lowFinal = w52low || (prof?.range ? parseFloat(prof.range.split('-')[0]) : 0);
  const w52highFinal = w52high || (prof?.range ? parseFloat(prof.range.split('-')[1]) : 0);
  const volFinal = vol || fmpQuote?.volume || 0;

  // Market Cap
  let mktCap = fmpQuote?.marketCap || prof?.marketCap || meta.marketCap || 0;
  const sharesOut = fmpQuote?.sharesOutstanding || meta.sharesOutstanding || 0;
  if (!mktCap && liveKursFinal && sharesOut) mktCap = liveKursFinal * sharesOut;

  const sektor = prof?.sector || 'n/a';
  const industrie = prof?.industry || 'n/a';
  ...
  if (roe === 'n/a' && inc0?.netIncome && inc0?.revenue) {
    const nm = inc0.netIncome / inc0.revenue;
    roe = (nm * 100 * 2).toFixed(1) + '% (gesch.)';
  }
  ...
  let macdTrend='n/a', bbSignal='n/a';
  let bbUpper='n/a', bbMiddle='n/a', bbLower='n/a';
  let macdVal='n/a', macdSignalVal='n/a', macdHistVal='n/a';
  try {
    ...
    if (c.length >= 20) {
      ...
      bbSignal=lc>(sma+2*std)?'Oberes Band':lc<(sma-2*std)?'Unteres Band':'Innerhalb der Bands';
    }
  } catch(e){}

  const mktCapStr = fmtUSD(mktCap);
  const sharesStr = sharesOut ? fmt(sharesOut)+' Stk' : 'n/a';

  const prompt = `Du bist Aktienanalyst fuer CapitalCrew. Erstelle Factsheet fuer ${t}.

ECHTE DATEN:
Kurs: ${liveKursFinal} ${currency} | 52W: ${w52lowFinal}-${w52highFinal} | MarketCap: ${mktCapStr}
Boerse: ${exchange} | Sektor: ${sektor} | HQ: ${hq} | CEO: ${ceo} | MA: ${employees} | IPO: ${ipoDate} | Beta: ${beta}
KGV: ${pe}x | P/S: ${ps}x | P/B: ${pb}x
...
Earnings: ${earningsStr} | Shares: ${sharesStr}
RSI: ${rsi} - ${rsiSignal} | MACD: ${macdTrend} | Bollinger: ${bbSignal}
NEWS: ${articles.map((a,i)=>`[${a.publishedAt?.slice(0,10)}] ${a.title}`).join(' | ')}

Antworte NUR mit validem JSON ohne Markdown und ohne Zeilenumbrueche in Strings:
{"ticker":"${t}","companyName":"${longName}","exchange":"${exchange}","sector":"${sektor}","hq":"${hq}","ceo":"${ceo}","employees":"${employees}","stand":"${today}","score":65,"verdict":"Urteil","verdictText":"3-4 Saetze Deutsch.","warning":"Risiken oder leer","kurs":"${liveKursFinal} ${currency}","w52low":"${w52lowFinal} ${currency}","w52high":"${w52highFinal} ${currency}","w52lowRaw":${w52lowFinal},"w52highRaw":${w52highFinal},"kursRaw":${liveKursFinal},"marketcap":"${mktCapStr}","volume":"${(volFinal/1e6).toFixed(1)} Mio.","beta":"${beta}","analystPT":"schaetzen USD","rsi":"${rsi}","rsiSignal":"${rsiSignal}","macd":"${macdVal}","macdSignal":"${macdSignalVal}","macdHist":"${macdHistVal}","macdTrend":"${macdTrend}","bbUpper":"${bbUpper}","bbMiddle":"${bbMiddle}","bbLower":"${bbLower}","bbSignal":"${bbSignal}","shortInterest":"schaetzen","instOwnership":"schaetzen","earnings":"${earningsStr}","chips":[{"label":"CEO","value":"${ceo}"},{"label":"Sitz","value":"${hq}"},{"label":"Mitarbeiter","value":"${employees}"},{"label":"IPO","value":"${ipoDate}"},{"label":"Stand","value":"${today}"}],"kpis":{"bewertung":[{"k":"Market Cap","v":"${mktCapStr}","color":""},{"k":"EV (gesch.)","v":"schaetzen","color":""},{"k":"KGV (TTM)","v":"${pe}x","color":""},{"k":"P/S (TTM)","v":"${ps}x","color":""},{"k":"P/B (TTM)","v":"${pb}x","color":""}],"pl":[{"k":"Umsatz TTM","v":"${revenue}","color":"pos"},{"k":"Umsatz YoY","v":"${revenueYoY}","color":"pos"},{"k":"Nettomarge","v":"${netMargin}","color":"pos"},{"k":"EPS (TTM)","v":"${eps} USD","color":"pos"},{"k":"EBITDA","v":"${ebitda}","color":"pos"}],"bilanz":[{"k":"Current Ratio","v":"${currentRatio}x","color":""},{"k":"D/E Ratio","v":"${deRatio}x","color":""},{"k":"ROE","v":"${roe}","color":"pos"},{"k":"FCF/Share","v":"${fcfPS} USD","color":"pos"},{"k":"Div. Rendite","v":"${divYield}","color":""}],"kapital":[{"k":"D/E Ratio","v":"${deRatio}x","color":""},{"k":"Current Ratio","v":"${currentRatio}x","color":""},{"k":"Div. Rendite","v":"${divYield}","color":""},{"k":"ROE","v":"${roe}","color":"pos"},{"k":"FCF/Share","v":"${fcfPS} USD","color":"pos"}],"aktie":[{"k":"RSI (14T)","v":"${rsi} - ${rsiSignal}","color":""},{"k":"MACD Trend","v":"${macdTrend}","color":""},{"k":"Bollinger","v":"${bbSignal}","color":""},{"k":"Beta","v":"${beta}","color":""},{"k":"Earnings","v":"${earningsStr}","color":""}]},"projekte":[{"name":"schaetzen","standort":"Global","kapazitaet":"schaetzen","status":"schaetzen","details":"schaetzen","kapColor":"pos"},{"name":"schaetzen","standort":"schaetzen","kapazitaet":"schaetzen","status":"schaetzen","details":"schaetzen","kapColor":""}],"news":[${newsItems.join(',')}],"insider":"schaetzen","dividenden":"schaetzen","bullCase":"schaetzen","baseCase":"schaetzen","bearCase":"schaetzen","chancen":["schaetzen","schaetzen","schaetzen","schaetzen","schaetzen","schaetzen"],"risiken":["schaetzen","schaetzen","schaetzen","schaetzen","schaetzen","schaetzen"],"szenarios":[{"name":"Bull","color":"pos","gew":"25%","annahmen":"schaetzen","fv":"schaetzen USD","begruendung":"schaetzen"},{"name":"Base","color":"gold","gew":"50%","annahmen":"schaetzen","fv":"schaetzen USD","begruendung":"schaetzen"},{"name":"Bear","color":"neg","gew":"25%","annahmen":"schaetzen","fv":"schaetzen USD","begruendung":"schaetzen"}],"fairValueGew":"schaetzen USD","fairValueCalc":"schaetzen","fairValueNote":"Kurs: ${liveKursFinal} USD","kapEigenkapital":[{"k":"Shares out.","v":"${sharesStr}","color":""},{"k":"Buybacks","v":"schaetzen","color":"pos"},{"k":"Dividende","v":"${divYield}","color":""},{"k":"Beta","v":"${beta}","color":""},{"k":"D/E Ratio","v":"${deRatio}x","color":""}],"kapFremdkapital":[{"k":"Gesamtschulden","v":"schaetzen","color":""},{"k":"Net Debt","v":"schaetzen","color":""},{"k":"D/E Ratio","v":"${deRatio}x","color":""},{"k":"Current Ratio","v":"${currentRatio}x","color":"pos"},{"k":"ROE","v":"${roe}","color":"pos"}],"quellen":"Yahoo Finance, FMP Stable API, NewsAPI"}`;
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages',{
      ...
      body:JSON.stringify({model:'claude-sonnet-4-6',max_tokens:4000,
        system:'Aktienanalyst. Antworte NUR mit validem JSON. Kein Markdown. Keine Zeilenumbrueche in Strings. Alle schaetzen-Felder realistisch fuellen.',
