export default async function handler(req, res) {
  ...
  const [yahooRaw, profileRaw, ratiosRaw, incomeRaw, analystRaw,
    ...
    safe(fetch(`https://financialmodelingprep.com/stable/grades-summary?symbol=${t}&apikey=${FMP}`).then(r=>r.json())),
  ...
  // Kurs — Yahoo erst, dann FMP Quote, dann FMP Profile
  ...
  const volFinal = vol || fmpQuote?.volume || prof?.volume || 0;

  let mktCap = fmpQuote?.marketCap || prof?.marketCap || meta.marketCap || summaryDetail?.marketCap?.raw || 0;
  const sharesOutstanding = fmpQuote?.sharesOutstanding || keyStats?.sharesOutstanding?.raw || meta.sharesOutstanding || 0;
  if (!mktCap && liveKursFinal && sharesOutstanding) mktCap = liveKursFinal * sharesOutstanding;
...
Kurs: ${liveKursFinal} ${currency} | 52W: ${w52lowFinal}-${w52highFinal} | MarketCap: ${mktCapStr} | Shares: ${sharesStr}
