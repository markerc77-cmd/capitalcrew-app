export default async function handler(req, res) {
  ...
  const [yahooRaw, fmpProfileRaw, fmpMetricsRaw, fmpIncomeRaw, fmpAnalystRaw,
    ...
    safe(fetch(`https://financialmodelingprep.com/stable/analyst-estimates?symbol=${t}&limit=1&apikey=${FMP}`).then(r=>r.json())),
    // FMP stable — Insider Trades
  ...
  const netMargin = inc0?.netIncomeRatio ? (inc0.netIncomeRatio * 100).toFixed(1) + '%' : 'n/a';
  const revenueYoY = inc0 && inc1 && inc1.revenue ? fmtPct(((inc0.revenue - inc1.revenue) / Math.abs(inc1.revenue)) * 100) : 'n/a';

  // ── FMP ANALYST (stable) ──
  const analystArr = Array.isArray(fmpAnalystRaw) ? fmpAnalystRaw : [];
  const an = analystArr[0] || null;
  const analystBuy = an?.numAnalystEstimates || 0;
  const analystStr = analystBuy > 0 ? `${analystBuy} Analysten | Est. EPS: ${an?.estimatedEpsAvg?.toFixed(2) || 'n/a'}` : 'n/a';

  // ── FMP INSIDER (stable) ──
  const insiderArr = Array.isArray(fmpInsiderRaw) ? fmpInsiderRaw.slice(0, 3) : [];
  const insiderStr = insiderArr.length > 0
    ? insiderArr.map(i => `${i.transactionDate||i.filingDate}: ${i.reportingName||i.name} ${i.transactionType||i.acquistionOrDisposition} ${Number(i.securitiesTransacted||i.shares||0).toLocaleString('de-DE')} Aktien`).join(' | ')
    : 'Keine aktuellen Insider-Transaktionen';

  // ── FMP EARNINGS (stable) ──
  const earnArr = Array.isArray(fmpEarningsRaw) ? fmpEarningsRaw : [];
  ...
  // ── FMP DIVIDENDEN (stable) ──
  const divArr = Array.isArray(fmpDividendRaw) ? fmpDividendRaw.slice(0, 3) : [];
  const divStr = divArr.length > 0
    ? divArr.map(d => `${d.date}: ${d.dividend?.toFixed(2) || d.adjDividend?.toFixed(2)} USD`).join(' | ')
    : 'Keine Dividende';
  ...
  try {
    ...
    if (c.length >= 20) {
      ...
      function ema(data, period) {
        ...
        for (let i = period; i < data.length; i++) e = data[i]*k + e*(1-k); return e;
      }
      if (c.length >= 35) {
  ...
  let instPct = 'n/a';
  try {
    const ir = await fetch(`https://financialmodelingprep.com/stable/institutional-ownership/institutional-holders?symbol=${t}&apikey=${FMP}`);
    const ij = await ir.json();
    if (Array.isArray(ij) && ij.length > 0) {
      const total = ij.reduce((a,b) => a+(b.shares||b.sharesNumber||0), 0);
...
KGV: ${pe}x | P/S: ${ps}x | P/B: ${pb}x | EV: ${ev} | Beta: ${beta} | DCF: ${analystPT}
